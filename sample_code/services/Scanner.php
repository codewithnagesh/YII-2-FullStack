...
namespace global\translate\services;

use Yii;
use yii\helpers\Console;
use global\translate\models\LanguageSource;
...
class Scanner
{
     ...
     */
    public $scanners = [
        '\global\translate\services\scanners\ScannerPhpFunction',
        '\global\translate\services\scanners\ScannerPhpArray',
        '\global\translate\services\scanners\ScannerJavaScriptFunction',
        '\global\translate\services\scanners\ScannerDatabase',
    ];
    ...
    public function run()
    {
        $scanTimeLimit = Yii::$app->getModule('translate')->scanTimeLimit;

        if (!is_null($scanTimeLimit)) {
            set_time_limit($scanTimeLimit);
        }

        $scanners = Yii::$app->getModule('translate')->scanners;
        if (!empty($scanners)) {
